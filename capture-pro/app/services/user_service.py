"""User service — register, login, subscription."""
import os
import hmac
import hashlib
import secrets
...
def register_user(email, password, name="", phone="", telegram_id=""):
    ...
    # Bikin user dengan trial dari env
    now = datetime.utcnow()
    
    # Baca durasi trial dari env (dalam menit)
    trial_minutes = int(os.environ.get("TRIAL_MINUTES", "10"))
    ...
    user = User(
        ...
        trial_ends_at=now + timedelta(seconds=trial_seconds),
        api_quota=100,
    )
    db.session.add(user)
    db.session.commit()
